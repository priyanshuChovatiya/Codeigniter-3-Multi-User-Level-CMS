...
class CityModel extends CI_Model
{
	...
	public function getCity($postData = null)
	{
		...
		$totalRecords = $this->db->select('*')->from('city')->where('user_id', $user_id)->get()->num_rows();


		## Total number of record with filtering
		$totalRecordwithFilter = $this->db->select('*')->from('city')->where('user_id', $user_id)->get()->num_rows();

		## Fetch records
		$records = $this->db->select('*')->from('city')->where('user_id', $user_id)
		...
		foreach ($records as $record) {
			...
			$link = base_url('admin/city/edit/') . $id;
